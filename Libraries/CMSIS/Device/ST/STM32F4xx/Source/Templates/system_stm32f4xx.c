/**
  ...
  * @version V1.5.0
  * @date    06-March-2015
  * @brief   CMSIS Cortex-M4器件外设访问层系统源文件。该文件包含STM32F4xx器件的系统时钟配置。
  *             
  * 1.  此文件提供了两个函数和一个要从用户应用程序调用的全局变量：
  *      - SystemInit(): 设置系统时钟 (系统时钟源, PLL_M: PLL倍频器
  *                      和分频器因子, AHB / APBx预分频器和flash(闪存)设置),
  *                      具体取决于时钟xls工具中的配置。 
  *                      在复位之后和进入主程序之前，调用此函数。 
  *                      此调用在“startup_stm32f4xx.s”文件中进行。
  *
  *      - SystemCoreClock变量：包含内核时钟（HCLK）,
  *                             用户应用程序可以使用它来设置SysTick定时器或配置其他参数。
  *                                     
  *      - SystemCoreClockUpdate(): 更新变量SystemCoreClock,
  *                               只要在程序执行期间更改内核时钟，就必须调用它。
  *
  * 2. 每次设备复位后，HSI（16 MHz）用作系统时钟源。
  *    然后在“startup_stm32f4xx.s”文件中调用SystemInit（）函数，在进入到主程序之前配置系统时钟。
  *
  * 3. 如果用户选择的系统时钟源无法启动，则SystemInit（）函数将不执行任何操作，
  *    HSI仍用作系统时钟源。 
  *    用户可以在SetSysClock（）函数中添加一些代码来处理这个问题。
  *
  * 4. HSE晶振的默认值设置为25MHz，请参考“stm32f4xx.h”文件中的“HSE_VALUE”定义。
  *    当HSE直接或通过PLL用作系统时钟源时，如果使用不同的晶振，则必须根据自己的配置调整HSE值。
  *
  * 5. 该文件配置系统时钟如下：
  *=============================================================================
  *=============================================================================
  ...
  *        APB2 Prescaler                         | 2
  *-----------------------------------------------------------------------------
  *        HSE Frequency(Hz)                      | 25000000  --> 修改为20M
  *-----------------------------------------------------------------------------
  *        PLL_M                                  | 25        --> 修改为20
  *-----------------------------------------------------------------------------
  *        PLL_N                                  | 336
...
void SystemInit(void)
{
  ...
  #endif
  /* Reset the RCC clock configuration to the default reset state 将RCC时钟配置默认的重置状态 ------------*/
  ...
  /* Reset HSEON, CSSON and PLLON bits 复位HSEON，CSSON和PLLON位 
		HSEON --> bit16 = 0 HSE 振荡器关闭
		CSSON --> bit19 =0 时钟安全系统使能配置 
		PLLON --> bit24 =0 主PLL使能配置
	*/
  RCC->CR &= (uint32_t)0xFEF6FFFF;

  /* Reset PLLCFGR register 复位PLLCFGR寄存器 PLL配置寄存器*/
  RCC->PLLCFGR = 0x24003010;

  /* Reset HSEBYP bit18 = 0  HSE时钟旁路 */
  RCC->CR &= (uint32_t)0xFFFBFFFF;

  /* Disable all interrupts 失能所有中断*/
  RCC->CIR = 0x00000000;
...
#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F401xx) || defined(STM32F446xx)
/******************************************************************************/
/*            PLL (clocked by HSE) used as System clock source   PLL（由HSE提供时钟）用作系统时钟源              */
/******************************************************************************/
  ...
  /* Wait till HSE is ready and if Time out is reached exit 等到HSE准备就绪，如果达到超时则退出 */
  do
  {
    HSEStatus = RCC->CR & RCC_CR_HSERDY;//时钟就绪标志 = 1
    StartUpCounter++;
  } while((HSEStatus == 0) && (StartUpCounter != HSE_STARTUP_TIMEOUT));

  if ((RCC->CR & RCC_CR_HSERDY) != RESET)
  {
    HSEStatus = (uint32_t)0x01;//时钟就绪标志 = 1
  }
  else
  {
  ...
  { /* 如果HSE无法启动，则应用程序将配置错误的时钟。 用户可以在这里添加一些代码来处理这个错误 */
	   while(1);
  }
#elif defined(STM32F411xE)
#if defined(USE_HSE_BYPASS) 
